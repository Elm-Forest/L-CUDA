...
__global__ void trace_kernel_standard(const T *input, T *output, size_t cols, size_t n){
    ...
    // Shared Memory 归约
    // volatile 确保跨warp可见性，兼容国产平台（如天数、摩尔）
...
T trace(const std::vector<T> &h_input, size_t rows, size_t cols){
    ...
    musaMalloc(&d_input, h_input.size() * sizeof(T));
    musaMalloc(&d_result, sizeof(T));

    musaMemcpy(d_input, h_input.data(), h_input.size() * sizeof(T), musaMemcpyHostToDevice);

    trace_kernel_standard<<<1, 256>>>(d_input, d_result, cols, n);

    musaMemcpy(&h_result, d_result, sizeof(T), musaMemcpyDeviceToHost);
...
void flashAttention(const std::vector<T> &h_q, const std::vector<T> &h_k,
    ...
    musaMalloc(&d_k, k_size);
    musaMalloc(&d_v, v_size);
    musaMalloc(&d_o, o_size);

    musaMemcpy(d_q, h_q.data(), q_size, musaMemcpyHostToDevice);
    musaMemcpy(d_k, h_k.data(), k_size, musaMemcpyHostToDevice);
    musaMemcpy(d_v, h_v.data(), v_size, musaMemcpyHostToDevice);
    ...
    musaMemcpy(h_o.data(), d_o, o_size, musaMemcpyDeviceToHost);
